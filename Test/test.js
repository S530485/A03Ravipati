...
QUnit.test('Testing Simple Calculator', function (assert) {
    assert.equal(cal(5, 5,1), 10, 'Tested with two relatively small positive numbers for addition');
    assert.equal(cal(6, 5,2), 1, 'Tested with two relatively small positive numbers for subtraction');
    assert.equal(cal(10, 9,3), 90, 'Tested with two relatively small positive numbers for multiplication');
    assert.equal(cal(40, 5,4), 8, 'Tested with two relatively small positive numbers for division');
    assert.equal(cal(-5, -5,1), -10, 'Tested with two relatively small negative numbers for addition');
    assert.equal(cal(-6, -5,2), -1, 'Tested with two relatively small negative numbers for subtraction');
    assert.equal(cal(-10, -9,3), 90, 'Tested with two relatively small negative numbers for multiplication');
    assert.equal(cal(-10, -5,4), 2, 'Tested with two relatively small negative and positive numbers for division');
    assert.equal(cal(-5, 5,1), 0, 'Tested with two relatively small negative and positive numbers for addition');
    assert.equal(cal(-6, 5,2), -11, 'Tested with two relatively small negative and positive numbers for subtraction');
    assert.equal(cal(-10, 9,3), -90, 'Tested with two relatively small negative and positive numbers for multiplication');
    assert.equal(cal(-10, 5,4), -2, 'Tested with two relatively small negative and positive numbers for division');
    assert.equal(cal(0, 0,3), 0, 'Tested with two zeros for multiplication');
    assert.throws(function () { cal(NaN,NaN,1); }, /The given argument is not a number/, 'Passing in NaN correctly raises an Error');
});
